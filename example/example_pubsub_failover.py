"""
Test that keeps an active pub/sub connection while killing the master.
This proves whether redis-py Sentinel handles failover transparently or if errors occur.
"""
import asyncio
import logging
import subprocess
...
# Enable INFO logging to see retry delay messages
logging.basicConfig(
    level=logging.INFO,
...
async def main():
    ...
    # Map port to container name (based on docker-compose.yaml)
    port_to_container = {
    ...
    async with broadcast.subscribe("test-channel") as subscriber:
        ...
        for i in range(3):
            await broadcast.publish("test-channel", f"message-{i}")
        ...
        # Try to continue pub/sub after failover
        ...
        for i in range(5):
            try:
                print(f"\nAttempt {i+1}:")
                await broadcast.publish("test-channel", f"after-failover-{i}")
